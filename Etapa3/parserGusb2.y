...
class ParserGusb
	...
	DECLARACION
		...
		;
	DECLA_CARD
		: IDENTIFICADOR ',' DECLA_CARD ',' TIPO  			{result = Decla_Card.new(val[0],nil,val[4],nil,val[2])}
		| IDENTIFICADOR ',' IDENTIFICADOR ':' TIPO ',' TIPO {result = Decla_Card.new(val[0],val[2],val[4],val[6],nil)}
		;
	DECLA_TIPO
		: IDENTIFICADOR ':' TIPO 		{result = Decla_Tipo.new(val[0],nil,val[2])}
		| IDENTIFICADOR ',' DECLA_TIPO  {result = Decla_Tipo.new(val[0],val[2],nil)}
		;
	# Reglas para reconocer una lista de tipos

	# Reglas para reconocer un tipo
	TIPO
		: 'int' 		{ result = TipoNum.new(val[0]) }
		| 'bool'    	{ result = TipoBool.new(val[0])  }
		| 'array' '[' LITERAL_NUMERO '..' LITERAL_NUMERO ']'	{ result = TipoArray.new(val[0],val[2],val[4]) }
		;
